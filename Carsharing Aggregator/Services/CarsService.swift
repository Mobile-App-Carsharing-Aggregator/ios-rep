...
extension CarsService {
     ...
     func getMockCars() -> [Car] {
        var cars: [Car] = []
        let carsharingCompany = [
            CarsharingCompany(name: "Яндекс Драйв"),
            CarsharingCompany(name: "Сити мобиль"),
            CarsharingCompany(name: "Супер Каршеринг компани")]
        let engineTypes: [EngineType] = [.diesel, .electro, .benzine]
        let carTypes: [CarType] = [.sedan, .hatchback, .minivan, .coupe, .universal, .other]
        
        for (index, location) in carsLocations.prefix(10).enumerated() {
                let car = Car(
                    id: UUID(),
